...
exports.register = (server, options, next) => {
    ...
    server.ext('onPreAuth', (request, reply) => {
        ...
        if (routeLimit) {
						const ip = request.headers['x-forwarded-for'] ? request.headers['x-forwarded-for'].split(',')[0] : request.info.remoteAddress;
            const ipts = [settings.namespace, ip, route.path].join(':');
